     ...
     <div className="d-sm-flex align-items-center custom-table-hdr text-center">
      <label className="d-flex align-items-center custom-select-label">
       ...
       <select
        ...
        defaultValue={option.sizePerPage}
        onChange={tableCall}
       >
           ...
           {column?.options?.sort &&
            (column.value !== option["sort"] ? (
             <>
              <button
               type="button"
               className="border-0 bg-transparent p-0 sorting-top"
              >
               <svg
                width="10"
                height="7"
               ...
               >
                <path
                 d="M0 4.95L1.413 6.365L1.414 6.364L4.95 2.829L8.486 6.363L9.894 4.955L4.95 0L0 4.95Z"
                 fill="#7B838A"
                />
               </svg>
              </button>
              <button
               type="button"
               className="border-0 bg-transparent p-0 sorting-bottom"
              >
               <svg
                width="10"
                height="7"
               ...
               >
                <path
                 d="M4.94988 6.80055L0.00488281 1.84455L1.41288 0.435547L4.94988 3.97055L8.48488 0.435547L9.89988 1.84955L4.94988 6.80055Z"
                 fill="#7B838A"
                />
               </svg>
              </button>
            ...
            return (
             <td key={index}>
              {col.options.customBodyRender
               ? col.options.customBodyRender(data, i)
               : data[i][col.value]}
             </td>
            );
           })}
      ...
      <div className="row align-items-center custom-table-btm text-center">
       <div className="col-md-4 text-md-start p-0">
        <div className="custom-table-page">
         Showing {parseInt((currentPage - 1) * option.sizePerPage + 1)} to{" "}
         {parseInt(
          (currentPage - 1) * option.sizePerPage + option.sizePerPage
         ) > option.totalRecord
          ? option.totalRecord
          : parseInt(
             (currentPage - 1) * option.sizePerPage + option.sizePerPage
            )}{" "}
         of {option.totalRecord} entries
        </div>
       </div>
       ...
       <div className="col-md-8 pt-3 pt-md-0 ms-md-auto text-md-end text-center p-0">
        <div className="pagination-custom-info">
         <ReactPaginate
          ...
          pageCount={Math.ceil(option.totalRecord / option.sizePerPage)}
